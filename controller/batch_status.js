...
const queries = require('../model/batch_statusModel.js')
...
router.post('/', async(req,res)=>{
  try{
    mySqlConnection.query(queries.INSERT_QUERY, [req.body.status,req.body.id], function (err, result) {
      if (err) throw err;
      console.log(result);
      res.json(result);
    });
  ...
  router.put('/', async(req,res)=>{
    try{
      mySqlConnection.query(queries.UPDATE_QUERY, [req.body.status,req.body.id], function (err, result) {
        if (err) throw err;
        console.log(result);
        res.json(result);
      });
