...
const queries = require('../model/courseModel.js')
...
router.post('/', async(req,res)=>{
  try{
    mySqlConnection.query(queries.INSERT_QUERY, [req.body.name,], function (err, result) {
      if (err) throw err;
      console.log(result);
      res.json(result);
    });
...
router.put('/', async(req,res)=>{
  try{
    mySqlConnection.query(queries.UPDATE_QUERY, [req.body.name,req.body.id], function (err, result) {
      if (err) throw err;
      console.log(result);
      res.json(result);
    });
